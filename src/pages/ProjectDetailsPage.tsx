...
const projectData: Record<string, { name: string; roi: string; duration: string; risk: string; raised: number; description: string }> = {
  "1": { name: "Green Valley Soy", roi: "18.5%", duration: "6 months", risk: "Low", raised: 72, description: "Premium soybean digital farming project located in the fertile Green Valley region. Sustainable practices with verified yields." },
  "2": { name: "Sunrise Maize Field", roi: "22.0%", duration: "4 months", risk: "Medium", raised: 45, description: "Fast-cycle maize project with competitive returns. Ideal for short-term digital farming investors." },
  "3": { name: "Cassava Digital Farm", roi: "15.2%", duration: "8 months", risk: "Low", raised: 88, description: "Established cassava farm with consistent yields and strong community backing." },
  "4": { name: "Rice Paddy Project", roi: "20.0%", duration: "5 months", risk: "Medium", raised: 60, description: "Rice cultivation project leveraging AI monitoring for optimal yield." },
  "5": { name: "Cocoa Bean Estate", roi: "25.0%", duration: "12 months", risk: "High", raised: 15, description: "Premium cocoa estate with high returns. Long-term investment with significant upside." },
};
          ...
          <Card className="border-0 shadow-lg">
            <CardContent className="p-4 grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <p className="text-lg font-extrabold text-foreground">{project.roi}</p>
                <p className="text-[10px] text-muted-foreground">Target ROI</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-lg font-extrabold text-foreground">{project.risk}</p>
                  ...
                  {supplyChain.map((s, i) => (
                    <div key={s.step} className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                        ...
                        s.status === "active" ? "bg-accent text-accent-foreground" :
                        "bg-muted text-muted-foreground"
                      }`}>
                        {s.status === "completed" ? <CheckCircle className="h-4 w-4" /> : i + 1}
                      </div>
                      ...
                      {i < supplyChain.length - 1 && (
                        <div className="absolute left-[35px] h-4 w-px bg-border" />
                      )}
                    </div>
                  ))}
                </div>
